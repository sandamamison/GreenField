...
public class CommandeService {
    ...
    // =========================================================
    @Transactional
    public CommandeModel creerCommande(CommandeRequest req) {

        // 1. Vérifier le client
        ClientModel client = clientRepository.findById(req.getClientId())
        ...
        for (CommandeRequest.LigneCommande ligne : req.getLignes()) {
            ...
            ProduitModel produit = produitRepository.findById(ligne.getProduitId())
                    .orElseThrow(
                            () -> new IllegalArgumentException("Produit introuvable : id=" + ligne.getProduitId()));
        ...
        if (req.getModeReception() == ModeReception.Retrait_Boutique) {
            PointDeVenteModel pdv = pointDeVenteRepository.findById(req.getPointDeVenteRetraitId())
    ...
    public Optional<CommandeModel> getCommandeById(Integer id) {
        return commandeRepository.findById(id);
    }

    public List<CommandeModel> getCommandesByClient(Integer clientId) {
